...
#[command(
    ...
    about = "Production-hardened clipboard-to-file tool with smart version management"
)]
struct Cli {
    #[command(subcommand)]
...
impl App {
    ...
    fn print_version(&self) {
        println!("PT version {}", self.version);
        println!("Production-hardened clipboard to file tool");
        println!("Features: Recursive search, backup management, delta diff, tree view, safe delete, configurable");
        println!();
        ...
        for path in version_paths {
            if path.exists() {
                if let Ok(abs_path) = path.canonicalize() {
                    println!("Version file: {}", abs_path.display());
                    break;
                }
            }
        }
        ...
        match Self::find_config_file() {
            ...
            None => println!("Config: Using defaults (no config file)"),
        }
    }
    ...
    fn find_config_file() -> Option<PathBuf> {
        ...
        // Add home directory paths (cross-platform)
        if let Some(home) = dirs::home_dir() {
            search_paths.push(home.clone());
            search_paths.push(home.join(".pt"));
        ...
        #[cfg(target_os = "windows")]
        {
            if let Ok(appdata) = env::var("APPDATA") {
                search_paths.push(PathBuf::from(&appdata));
                search_paths.push(PathBuf::from(&appdata).join(".pt"));
            }
            if let Ok(userprofile) = env::var("USERPROFILE") {
                search_paths.push(PathBuf::from(&userprofile));
                search_paths.push(PathBuf::from(&userprofile).join(".pt"));
            }
        }
        ...
        -> Result<(), Box<dyn std::error::Error>> 
    {
        ...
        if !comment.is_empty() {
            ...
            println!("📦 Backup created: {}{}{}", COLOR_GREEN, backup_name, COLOR_RESET);
            println!("💬 Comment: \"{}{}{}", COLOR_CYAN, comment, COLOR_RESET);
        } else {
            println!("📦 Backup created: {}{}{}", COLOR_GREEN, backup_name, COLOR_RESET);
        }
        ...
        -> Result<(), Box<dyn std::error::Error>> 
    {
        ...
        if check_mode && !append {
            if let Ok(existing) = fs::read_to_string(file_path) {
                if existing == data {
                    println!("ℹ️  Content identical to current file, no changes needed");
                    println!("📄 File: {}", file_path.display());
                    return Ok(());
                }
                println!("🔍 Content differs, proceeding with backup and write");
            }
        }
        ...
        println!("✅ Successfully {}: {}", action, file_path.display());
        println!("📄 Content size: {} characters", data.len());
    ...
    fn print_backup_table(&self, file_path: &Path, backups: &[BackupInfo]) {
        ...
        for (i, backup) in backups.iter().enumerate() {
            ...
            println!("{}│{} {:>3}. {:<width1$} {}│{} {:<width2$} {}│{} {:>width3$} {}│{} {:<width4$} {}│{}",
                COLOR_GRAY, COLOR_RESET,
                i + 1, name,
                COLOR_GRAY, COLOR_RESET,
                mod_time,
        ...
        -> Result<(), Box<dyn std::error::Error>> 
    {
        ...
        println!("✅ Successfully restored: {}", original_path.display());
        println!("📦 From backup: {}", backup_path.file_name().unwrap().to_string_lossy());
        println!("📄 Content size: {} characters", content.len());
        
        if !comment.is_empty() {
            println!("💬 Restore comment: \"{}\"", comment);
        }
    ...
    fn resolve_file_path(&self, filename: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        println!("{}🔍 Searching for '{}' in subdirectories...{}", COLOR_BLUE, filename, COLOR_RESET);
        ...
        if results.len() == 1 {
            println!("{}✓ Found: {}{}", COLOR_GREEN, results[0].path.display(), COLOR_RESET);
    ...
    fn handle_config_init(&self, path: Option<PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if config_path.exists() {
            println!("{}⚠️  Warning: Config file already exists: {}{}",
...
"#;
        ...
        println!("✅ Sample config file created: {}{}{}", COLOR_GREEN, config_path.display(), COLOR_RESET);
        println!("📝 Edit this file to customize PT behavior");
    ...
    fn handle_config_path(&self) {
        match Self::find_config_file() {
            Some(path) => {
                println!("📄 Config file: {}{}{}", COLOR_GREEN, path.display(), COLOR_RESET);
            }
            None => {
                println!("{}ℹ️  No config file found{}", COLOR_GRAY, COLOR_RESET);
                println!("\nSearched in:");
                println!("  • ./pt.yml or ./pt.yaml");
                println!("  • ~/.config/pt/pt.yml or ~/.config/pt/pt.yaml");
                println!("  • ~/pt.yml or ~/pt.yaml");
    ...
    fn handle_remove(&self, filename: &str, comment: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!("🗑️  File deleted: {}", file_path.display());

        // Create empty placeholder
        fs::File::create(&file_path)?;

        println!("📄 Created empty placeholder: {}", file_path.display());
        println!("ℹ️  Original content ({} bytes) backed up to ./{}/",
    ...
    fn handle_list(&self, filename: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if backups.is_empty() {
            println!("ℹ️  No backups found for: {} (check ./{}/ directory)",
    ...
    fn handle_diff(&self, filename: &str, use_last: bool) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let selected_backup = if use_last {
            println!("{}📊 Comparing with last backup: {}{}\n",
    ...
    fn handle_append(&self, filename: &str, comment: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if text.is_empty() {
            println!("{}⚠️  Warning: Clipboard is empty{}", COLOR_YELLOW, COLOR_RESET);
            return Ok(());
        }
    ...
    fn handle_write(&self, filename: &str, check_mode: bool, comment: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if check_mode {
            println!("🔍 Check mode enabled - will skip if content identical");
        }
